...
import edu.gatech.chai.omoponfhir.omopv5.stu3.provider.ImmunizationResourceProvider;
...
public class OmopImmunization extends BaseOmopResource<Immunization, DrugExposure, DrugExposureService>
	...
	public Immunization constructFHIR(Long fhirId, DrugExposure entity) {
		...
		// Set patient
		Reference patientReference = new Reference(new IdDt(PatientResourceProvider.getType(), entity.getFPerson().getId()));
		...
		// date
		immunization.setDate((new DateTimeType(entity.getDrugExposureStartDate())).getValue());
		
		// vaccine code
		CodeableConcept vaccineCodeable = CodeableConceptUtil.getCodeableConceptFromOmopConcept(entity.getDrugConcept(), getFhirOmopVocabularyMap());
		immunization.setVaccineCode(vaccineCodeable);
		
		// performer
		Provider provider = entity.getProvider();
		if (provider != null) {
			Reference performerReference = new Reference(new IdType(PractitionerResourceProvider.getType(), entity.getProvider().getId()));
//			performerReference.setDisplay(entity.getProvider().getProviderName());
			ImmunizationPractitionerComponent practitionerComponent = new ImmunizationPractitionerComponent();
			practitionerComponent.setActor(performerReference);
			immunization.addPractitioner(practitionerComponent);
		}
		
		// encounter
		VisitOccurrence visitOccurrence = entity.getVisitOccurrence();
		if (visitOccurrence != null) {
			Reference encounterReference = new Reference(new IdType(EncounterResourceProvider.getType(), entity.getVisitOccurrence().getId()));
		...
		if (routeConcept != null) {
			CodeableConcept routeCodeable = CodeableConceptUtil.getCodeableConceptFromOmopConcept(routeConcept, getFhirOmopVocabularyMap());
		...
		if (quantity != null && !quantity.isInfinite() && !quantity.isNaN()) {
			...
			simpleQuantity.setValue(quantity);
		...
		if (sig != null && !sig.isEmpty()) {
			List<Annotation> theValue = new ArrayList<Annotation> ();
	...
	public DrugExposure constructOmop(Long omopId, Immunization fhirResource) {
		...
		// performer
		ImmunizationPractitionerComponent paractitioner = fhirResource.getPractitionerFirstRep();
		if (!paractitioner.isEmpty()) {
			Reference performerReference = paractitioner.getActor();
		...
		// doseQuantity
		SimpleQuantity doseQuantity = fhirResource.getDoseQuantity();
